...
class KITTIData {
  public:
    ...
    void reproject_imgs(const int current_scan_id, la3dm::SemanticBGKOctoMap& map) {
    ...
    void reproject_img(const int scan_id, const int reproj_id, la3dm::SemanticBGKOctoMap& map) {
      Eigen::Matrix4f transform = get_current_pose(scan_id);
      for (int32_t i = 0; i < im_width_ * im_height_; ++i) {
        int ux = i % im_width_;
        int uy = i / im_width_;
        ...
        if (pix_depth > 0.1) {
          ...
          la3dm::SemanticOcTreeNode node = map.search(pt.x, pt.y, pt.z);
